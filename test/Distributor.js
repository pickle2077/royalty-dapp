...
describe("Token contract", function () {
  ...
  //   const Lottery = await ethers.getContractFactory("Lottery", signer);
  ...
  //     expect(await lottery.manager()).to.equal(signer.address);
  //   });
  // });
  ...
  describe("Lottery Contract", function () {
    it("allows one account to enter", async () => {
      ...
      const players = await lottery.connect(signer).getPlayers();
      console.log(players);
      expect(signer.address).to.equal(players[0]);
  ...
  //   expect(signer).to.equal(players[0]);
  ...
  //   const initialBalance = await ethers.provider.getBalance(signer);
  //   await lottery.connect(signer).pickWinner();
  //   const finalBalance = await ethers.provider.getBalance(signer);
